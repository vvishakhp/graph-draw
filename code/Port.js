/**
 * @class  .Port
 * A port is an object that is used to establish a connection between a node and a {@link  .Connection}. The port can
 * be placed anywhere within a node ( see {@link  .layout.locator.PortLocator} for details)
 *
 *
 * @author Andreas Herz
...
import   from 'packages'
 ...
 .Port =  .shape.basic.Circle.extend({
  NAME: " .Port",

  DEFAULT_BORDER_COLOR: new  .util.Color("#1B1B1B"),
  ...
  init: function (attr, setter, getter) {
    ...
    this.corona = null //  .shape.basic.Circle
    ...
    this.connectionAnchor = new  .layout.anchor.ConnectionAnchor(this)
    ...
    this.installEditPolicy(new  .policy.port.IntrusivePortsFeedbackPolicy())
    //    this.installEditPolicy(new  .policy.port.ElasticStrapFeedbackPolicy());
   ...
   *
   * @param { .layout.anchor.ConnectionAnchor} [anchor] the new source anchor for the connection or "null" to use the default anchor.
   **/
  setConnectionAnchor: function (anchor) {
    // set some good defaults.
    if (typeof anchor === "undefined" || anchor === null) {
      anchor = new  .layout.anchor.ConnectionAnchor()
    }
   ...
   * Possible values:
   * <ul>
   *   <li> .geo.Rectangle.DIRECTION_UP</li>
   *   <li> .geo.Rectangle.DIRECTION_RIGHT</li>
   *   <li> .geo.Rectangle.DIRECTION_DOWN</li>
   *   <li> .geo.Rectangle.DIRECTION_LEFT</li>
   * </ul>
   * <p>
   *
   * @param { .Port} peerPort the counterpart port
   *
   ...
   *
   * @param { .layout.locator.Locator} locator
   */
   ...
   *
   * @param { .util.Color|String} color The new background color of the figure
   **/
  ...
  /**
   * @method
   * Returns a {@link  .util.ArrayList} of {@link  .Connection}s of all related connections to this port.
   *
...
//        this.getShapeElement().insertAfter(this.parent.getShapeElement());
  ...
  onDragEnd: function (x, y, shiftKey, ctrlKey) {
    ...
    // port.
    //  .shape.basic.Rectangle.prototype.onDragEnd.call(this); DON'T call the super implementation!!!
   ...
   *      });
   *
   * @param { .Connection} connection The connection which has been created
   * @since 2.5.1
   *
   * @template
   **/
   ...
   *      });
   *
   * @param { .Connection} connection The connection which has been deleted
   * @since 2.5.1
   *
   * @template
   **/
   ...
   * The corona width can be set with {@link  .Port#setCoronaWidth}
   * @param {Number} iX
   * @param {Number} iY
   * @param {Number} [corona]
   * @return {Boolean}
  ...
  createCommand: function (request) {
    ...
    if (request.getPolicy() ===  .command.CommandType.MOVE) {
      ...
      return new  .command.CommandMovePort(this)
    }
 ...
 *
 * @extend  .shape.basic.Circle
 */
 .Corona =  .shape.basic.Circle.extend({
  ...
  init: function () {
    this._super()
    this.setAlpha(0.3)
    this.setBackgroundColor(new  .util.Color(178, 225, 255))
    this.setColor(new  .util.Color(102, 182, 252))
  },
